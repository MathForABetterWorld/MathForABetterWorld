...
import json
import os 
from routeConnectors import categoryConnectors, exportConnectors, locationConnectors, userConnector, employeeConnectors
...
if 'token' in st.session_state:
    ...
    employee_data = employeeConnectors.getEmployees()
    employee_dict = json.loads(employee_data)
    employees = employee_dict["employees"]
    employee_users = [employee["user"] for employee in employees]
    allUsers = sorted(users + employee_users, key=lambda u: u["name"])
    ...
    st.error("No access to query exports. Please log in if employee.")
